...
return new class extends Migration
{
    ...
    {
        Schema::connection('tenant')->table('settings', function (Blueprint $table) {
            // Only add columns if they don't exist
            if (!Schema::connection('tenant')->hasColumn('settings', 'penalty_type')) {
                $table->enum('penalty_type', ['monthly_recurring', 'one_time'])
                    ->default('monthly_recurring')
                    ->after('penalty_percentage');
            }
            
            if (!Schema::connection('tenant')->hasColumn('settings', 'monthly_penalty_rate')) {
                $table->decimal('monthly_penalty_rate', 5, 2)->default(5.00)
                    ->after('penalty_type');
            }
            
            if (!Schema::connection('tenant')->hasColumn('settings', 'one_time_penalty_rate')) {
