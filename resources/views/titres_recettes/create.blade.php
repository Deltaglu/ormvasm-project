...
@section('content')
<x-page-header title="Nouveau titre de recette" subtitle="Émettre un titre de recette avec lignes de prestations. Le montant total est calculé automatiquement.">
...
<div class="ormsa-surface ormsa-form-card">
    <div class="card-body">
        <form method="post" action="{{ route('titres-recettes.store') }}" class="row g-3" id="titreForm">
            @csrf
            
            <div class="col-md-6">
                <label class="form-label" for="numero">Numéro (auto-généré si vide)</label>
            ...
            <div class="col-md-6">
                ...
                <div class="form-text">Optionnel. Passée cette date, une pénalité est calculée sur le solde restant.</div>
            </div>
            ...
            <div class="col-md-6">
                ...
                <select name="agriculteur_id" id="agriculteur_id" class="form-select" required>
                    ...
                    @foreach($agriculteurs as $agriculteur)
                        <option value="{{ $agriculteur->id }}" @selected(old('agriculteur_id') == $agriculteur->id)>{{ $agriculteur->prenom }} {{ $agriculteur->nom }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="col-md-6">
                ...
                <input type="number" step="0.01" min="0" name="montant_total" id="montant_total" class="form-control" value="0.00" readonly>
                <div class="form-text">Calculé automatiquement à partir des prestations</div>
            </div>
            
            <div class="col-12">
                <label class="form-label" for="objet">Objet</label>
                <textarea name="objet" id="objet" class="form-control" rows="3">{{ old('objet') }}</textarea>
            </div>
