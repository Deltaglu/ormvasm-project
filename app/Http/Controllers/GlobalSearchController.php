...
class GlobalSearchController extends Controller
{
    public function search(Request $request): JsonResponse
    {
        ...
        foreach ($agris as $a) {
            $results[] = [
                'type' => $a->trashed() ? 'Agriculteur (Supprimé)' : 'Agriculteur',
                'icon' => 'bi-people',
        ...
        foreach ($paiements as $p) {
            $results[] = [
                'type' => $p->trashed() ? 'Paiement (Supprimé)' : 'Paiement',
                'icon' => 'bi-cash-stack',
        ...
        // 3. Titres
        $titres = TitreRecette::withTrashed()
            ->where('numero', 'LIKE', "%{$q}%")
            ->limit(5)
            ->get();
        foreach ($titres as $t) {
            $results[] = [
                'type' => $t->trashed() ? 'Titre de Recette (Supprimé)' : 'Titre de Recette',
                'icon' => 'bi-receipt',
                'title' => 'TR: ' . $t->numero,
